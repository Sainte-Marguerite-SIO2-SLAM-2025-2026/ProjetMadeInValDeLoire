...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : dim. 22 fév. 2026 à 21:41
...
(1, 'testtest', 0, NULL, NULL, NULL, 4, NULL, NULL, NULL, NULL, NULL),
...
DELIMITER $$
CREATE TRIGGER `Before_insert` BEFORE INSERT ON `activite` FOR EACH ROW BEGIN
IF new.salle_numero = 5 THEN
    IF LEFT(new.image, 17) <> "../images/salle_5/" THEN
    set new.image = CONCAT("../images/salle_5/", new.image);
    END IF;
END IF;
END
$$
...
DELIMITER $$
CREATE TRIGGER `Before_update` BEFORE UPDATE ON `activite` FOR EACH ROW BEGIN
IF new.salle_numero = 5 THEN
    IF LEFT(new.image, 17) <> "../images/salle_5/" THEN
    set new.image = CONCAT("../images/salle_5/", new.image);
    END IF;
...
-- Déclencheurs `objets`
--
DROP TRIGGER IF EXISTS `Before_insert_objet`;
DELIMITER $$
CREATE TRIGGER `Before_insert_objet` BEFORE INSERT ON `objets` FOR EACH ROW BEGIN
    IF LEFT(new.image, 15) <> "images/salle_5/" THEN
    set new.image = CONCAT("images/salle_5/", new.image);
    END IF;
END
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `Before_update_objets`;
DELIMITER $$
CREATE TRIGGER `Before_update_objets` BEFORE UPDATE ON `objets` FOR EACH ROW BEGIN
    IF LEFT(new.image, 15) <> "images/salle_5/" THEN
    set new.image = CONCAT("images/salle_5/", new.image);
    END IF;
END
...
--
-- Déclencheurs `objet_declencheur_enigme`
--
DROP TRIGGER IF EXISTS `Before_insert_objet_declencheur`;
DELIMITER $$
CREATE TRIGGER `Before_insert_objet_declencheur` BEFORE INSERT ON `objet_declencheur_enigme` FOR EACH ROW BEGIN
    IF LEFT(new.image_path, 15) <> "images/salle_5/" THEN
    set new.image_path = CONCAT("images/salle_5/", new.image_path);
    END IF;
END
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `Before_update_objets_declencheurs`;
DELIMITER $$
CREATE TRIGGER `Before_update_objets_declencheurs` BEFORE UPDATE ON `objet_declencheur_enigme` FOR EACH ROW BEGIN
    IF LEFT(new.image_path, 15) <> "images/salle_5/" THEN
    set new.image_path = CONCAT("images/salle_5/", new.image_path);
    END IF;
END
